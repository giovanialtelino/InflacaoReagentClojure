  ...
  (let [ano (-> js/document
    ...
    (if (= 4 (count ano))
      (let [ano-n (int ano)
        ...
        (if (tt/within? (tt/interval (tt/local-date 1970 5 15) (tt/local-date 1986 2 27)) data)
          (reset! moeda "Valor inteiro inicial para cálculo (Cr$)")
          (if (tt/within? (tt/interval (tt/local-date 1986 2 28) (tt/local-date 1989 1 15)) data)
            (reset! moeda "Valor inteiro inicial para cálculo (Cz$)")
            (if (tt/within? (tt/interval (tt/local-date 1989 1 16) (tt/local-date 1990 3 15)) data)
              (reset! moeda "Valor inteiro inicial para cálculo (NCz$)")
              (if (tt/within? (tt/interval (tt/local-date 1990 3 16) (tt/local-date 1993 7 30)) data)
                (reset! moeda "Valor inteiro inicial para cálculo (Cr$)")
                (if (tt/within? (tt/interval (tt/local-date 1993 8 1) (tt/local-date 1994 6 30)) data)
                  (reset! moeda "Valor inteiro inicial para cálculo (CR$)")
...
(defn dropdown-selector-mes [id]
  [:div.select-style
   [:select
    {:id (str "mes-" id)
     :on-change   #(check-change-mes-selector-valid (-> % .-target .-id))}
